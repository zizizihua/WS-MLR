...
def upsample_pos_emb(emb, new_size):
    ...
    # emb size NxD [197 4096]
    first = emb[:1, :] # [1 4096]
    emb = emb[1:, :] # [196 4096]
    N, D = emb.size(0), emb.size(1)
    size = int(np.sqrt(N)) # 14
    assert size * size == N 
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x, H, W):
        # print(f"this is input H : {H} & W : {W}")
        # print(f"this is original x : {np.shape(x)}") # [1 4096 13 18]
        # print(f"this is original pos : {np.shape(self.positional_embedding)}") # [197 4096]
        ...
        if W % 32 == 31:
            ...
            W_new = W//32
        self.positional_embedding_new = upsample_pos_emb(self.positional_embedding, (H_new,W_new)) # 13 18
        # print(f"this is x : {np.shape(x)}") # [235 1 4096]
        # print(f"this is new embed : {np.shape(self.positional_embedding_new[:, None, :])}") # [217 1 4096]
...
class CLIP(nn.Module):
    ...
    def forward_last_layer(self, image_features, text_features):
        x = self.visual.transformer.resblocks[self.visual.transformer.layers-1](image_features)
        x = x.permute(1, 0, 2)  # LND -> NLD
