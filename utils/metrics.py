...
def compute_pseudo_label_accuracy(meter, pseudoLabel, groundTruth, margin=0.5):
    ...
    meter['precision'].update(np.sum((pseudoLabel == 1).astype(float) * (groundTruth == 1).astype(float)), np.sum(pseudoLabel == 1))
    meter['recall'].update(np.sum((pseudoLabel == 1).astype(float) * (groundTruth == 1).astype(float)), np.sum(groundTruth == 1))
...
def compute_intra_cooccurrence_accuracy(meter, coOccurrence, target):
    ...
    target1, target2 = (target1 > 0).astype(float), (target2 > 0).astype(float)
    target = target1 * target2

    meter['accuracy'].update(np.sum(coOccurrence == target), target.shape[0] * target.shape[1])
    meter['precision'].update(np.sum((coOccurrence == 1).astype(float) * (target == 1).astype(float)), np.sum(coOccurrence == 1))
    meter['recall'].update(np.sum((coOccurrence == 1).astype(float) * (target == 1).astype(float)), np.sum(target == 1))
...
def compute_mAP(pred, target):
    ...
    for k in range(num_classes):
        ap[k] = average_precision(pred[:, k], target[:, k]) * 100
    return ap.mean(), ap
