...
from model.ML_GCN import GCNResnet, gen_A
...
def main():
    ...
    adj = gen_A(args.classNum, t=0.4, labels=train_loader.dataset.labels)
    model = GCNResnet(args.classNum, adj=adj)
    ...
    for epoch in range(args.startEpoch, args.startEpoch + args.epochs):
        ...
        save_checkpoint(args, {'epoch': epoch, 'optimizer': optimizer.state_dict(), 'model':model.state_dict()}, f'last')
