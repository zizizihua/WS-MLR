...
def compute_prototype(model1, model2, train_loader1, train_loader2, args):
    ...
    with torch.no_grad():
        ...
        for i in range(args.classNum):
            ...
            prototypes.append(torch.tensor(kmeans.cluster_centers_).to(args.deviceObj))
...
def main():
    ...
    model1.to(args.deviceObj)
    model2.to(args.deviceObj)
    ...
    criterion = {'BCELoss': nn.BCEWithLogitsLoss().to(args.deviceObj),
                 'CELoss': nn.CrossEntropyLoss().to(args.deviceObj)}
...
def Train(train_loader1, train_loader2, model1, model2, criterion, optimizer1, optimizer2, writer, epoch, args): #, predictions, similarities, labels):
    ...
    predictions = torch.zeros(queue_len, args.classNum, device=args.deviceObj)
    similarities = torch.zeros(queue_len, args.classNum, device=args.deviceObj)
    labels = torch.zeros(queue_len, args.classNum, device=args.deviceObj)
    q_idx = 0
